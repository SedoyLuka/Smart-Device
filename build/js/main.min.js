...
var init = function init() {
  ...
  accordionParts.forEach(function (accordionPart) {
    // Если не кнопка, по клавиатуре не работает
    accordionPart.addEventListener('keydown', function (evt) {
      if (evt.key === 'Enter') {
        accordionPart.click();
      }
    });
    accordionPart.addEventListener('click', function (evt) {
      if (!isMobile) {
        return;
      }
      accordionParts.forEach(function (accordionPart) {
        if (accordionPart === evt.currentTarget && accordionPart.dataset.accordionState === 'close') {
